...
pub trait Node<P> {
    ...
    where
        Self: Sized;
    fn step(&mut self, input: Message<P>, stdout: &mut StdoutLock) -> anyhow::Result<()>;
}
    ...
    N: Node<P>,
{
    ...
    for line in stdin {
        let line = line.context("Malestrom input from STDIN could not be deserialized")?;
